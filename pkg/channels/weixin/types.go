...
func (c *WeixinChannel) getTypingTicket(ctx context.Context, userID string) (string, error) {
	...
	if err == nil && resp != nil && resp.Ret == 0 && resp.Errcode == 0 {
		...
		logger.InfoCF("weixin", "GetConfig success, got typing ticket", map[string]any{
			"user_id":      userID,
			"has_ticket":   ticket != "",
	...
	// GetConfig 失败，记录详细信息
	if err != nil {
		logger.WarnCF("weixin", "GetConfig API call failed", map[string]any{
			"user_id": userID,
			"error":   err.Error(),
		})
	} else if resp != nil {
		logger.WarnCF("weixin", "GetConfig API returned error", map[string]any{
			"user_id": userID,
			"ret":      resp.Ret,
