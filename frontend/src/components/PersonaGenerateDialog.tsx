...
function StepIcon({ status }: { status: GenerateStepStatus }) {
  ...
  if (status === 'active') {
    return <IconLoader2 className="size-4 animate-spin text-violet-600" aria-hidden />;
  }
  if (status === 'error') {
...
export function PersonaGenerateDialog({
  ...
  const label = PERSONA_FILE_LABELS[fileName].title;

  return (
    <Dialog.Root open={open}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50 bg-black/40 supports-backdrop-filter:backdrop-blur-[2px] data-[state=open]:animate-in data-[state=open]:fade-in-0" />
        <Dialog.Content
          className={cn(
            'fixed left-1/2 top-1/2 z-50 w-[min(92vw,24rem)] -translate-x-1/2 -translate-y-1/2',
        ...
        >
          ...
          <Dialog.Description className="mt-1 text-xs text-muted-foreground">
            正在根据提示词生成 <span className="font-mono">{fileName}</span>
          </Dialog.Description>

          <div className="mt-3 rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
            <p className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">提示词</p>
            <p className="mt-1 line-clamp-3 text-xs leading-relaxed text-foreground/90">{prompt}</p>
          </div>
...
export const PERSONA_GENERATE_STEP_DEFS = [
  { id: 'validate', label: '校验提示词与文件类型' },
  { id: 'call', label: '调用 Agent 绑定的模型' },
  { id: 'finalize', label: '整理 Markdown 并写入编辑器' },
] as const;
